variable "app_name" {
  ...
  validation {
    condition     = can(regex("^[A-Za-z0-9][A-Za-z0-9_-]{1,17}$", var.app_name))
    error_message = "app_name must be 2-18 characters and contain only letters, numbers, hyphens, or underscores."
  }
}
